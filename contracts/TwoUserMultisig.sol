...
contract TwoUserMultisig is IAccount, IERC1271 {
    ...
    function validateTransaction(
        bytes32,
        bytes32 _suggestedSigndHash,
        Transaction calldata _transaction
    ) external payable override onlyBootloader returns (bytes4 magic) {
        magic = _validateTransaction(_suggestedSigndHash, _transaction);
    }
    ...
    function _validateTransaction(
        ...
        // Increamenting the nonce of the account
        ...
        bytes32 txHash;
        // While suggesting signed hash is usually provided, it is generally not recommended
        // to rely on it to be present, since in the future there may be tx types with no suggested hash
        ...
        // The fact there is enough balance for the account should be checked explicitly to prevent user
        // paying for fee fpr a transaction that wouldn't be included in Ethereum
        ...
        returns (bytes4 magic)
    {
        ...
        if (_signature.length != 130) {
            ...
            // in order for the fee estimation to work properly
            _signature = new bytes(130);

            // Making sure that the signatures look like a valid ECDSA signatures and are not rejected rightaway
